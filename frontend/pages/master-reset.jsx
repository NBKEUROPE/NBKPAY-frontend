...
export default function MasterReset() {
  ...
  const handleSubmit = e => {
    ...
    setTimeout(() => setMessage("Master reset successful!"), 1000);
  };
  return (
    <div className="glass-bg">
      <form className="glass-form" onSubmit={handleSubmit}>
        <img src="/logo-nbkpay.svg" alt="NBK Pay Logo" className="logo" />
        <div className="form-title">Master Password Reset</div>
        <label>
          Terminal ID
        ...
        <label>
          Master Password
          <input className="input" type="password" required value={masterPw} onChange={e => setMasterPw(e.target.value)} />
        </label>
        <div className="login-links">
          <Link href="/login" className="link">Back to Login</Link>
        </div>
        <button className="btn primary" type="submit">Master Reset</button>
