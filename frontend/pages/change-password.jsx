...
const terminals = Array.from({length: 1000}, (_, i) =>
  `NBK-PAY-${(i+1).toString().padStart(4, "0")}`
);
...
export default function ChangePassword() {
  ...
  const handleSubmit = e => {
    ...
    setTimeout(() => setMessage("Password changed!"), 900);
  };
    ...
    <div className="glass-bg">
      <form className="glass-form" onSubmit={handleSubmit}>
        ...
        {showOtp && (
          <label>
            OTP (if required)
            <input className="input" maxLength={6} value={otp}
              onChange={e => setOtp(e.target.value)}
            />
          </label>
        )}
        <div className="login-links">
          <button type="button" className="link" onClick={() => setShowOtp(!showOtp)}>
            {showOtp ? "Cancel OTP" : "Use OTP"}
          </button>
      ...
      <style jsx>{`
        ...
        .glass-form {
          width:99vw;max-width:410px;background:rgba(255,255,255,0.22);
          ...
          border:1.5px solid rgba(255,255,255,0.22);gap:18px;
        }
        ...
        .btn.primary {width:100%;background:linear-gradient(90deg,#d7263d 70%,#ffb4b4 100%);
          color:#fff;border:none;border-radius:8px;padding:13px 0 12px 0;
          font-size:1.09em;font-weight:600;cursor:pointer;margin-top:3px;}
